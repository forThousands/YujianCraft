...
public final class SpiritTrialDummyRenderer extends EntityRenderer<SpiritTrialDummyEntity> {
    // The aura extends substantially beyond the opaque blade tip. Keep the render origin high
    // enough that both the metal blade and its sword-qi terminate visibly above the lower slab.
    ...
    public void render(SpiritTrialDummyEntity pedestal, float yaw, float partialTick,
        ...
        renderSuspendedSword(pedestal, partialTick, poseStack, buffers, packedLight);
        poseStack.popPose();
    ...
    private void renderSlab(PoseStack poseStack, MultiBufferSource buffers, int packedLight,
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                BlockState state;
                if (x == 0 && z == 0) {
                    state = Blocks.PRISMARINE_BRICKS.defaultBlockState();
    ...
    private void renderCell(BlockState state, PoseStack poseStack, MultiBufferSource buffers,
        ...
        // Both pieces are half a block thick. The upper offset separates its underside from the
        // brightest sword-aura pixels and avoids z-fighting at glancing camera angles.
    ...
    private void renderSuspendedSword(SpiritTrialDummyEntity pedestal, float partialTick,
        ...
        float rotation = (pedestal.tickCount + partialTick) * 0.72F;
