    ...
    private static BooleanSupplier createProbe() {
        for (String apiClassName : new String[]{
            ...
            try {
                ...
                LOGGER.info("Swordflight shader adaptation is using {}", apiClassName);
                return () -> invokeBoolean(isShaderPackInUse, invoke(getInstance, null));
            } catch (ClassNotFoundException ignored) {
                // Try the next supported shader loader.
            } catch (ReflectiveOperationException exception) {
                LOGGER.warn("Could not initialize Swordflight shader detection through {}", apiClassName, exception);
        ...
        try {
            ...
            try {
                ...
                LOGGER.info("Swordflight shader adaptation is using OptiFine's shader method");
                ...
                LOGGER.info("Swordflight shader adaptation is using OptiFine's shader field");
                return () -> readBoolean(shaderPackLoaded);
            }
        } catch (ClassNotFoundException ignored) {
            LOGGER.info("No supported shader loader detected; Swordflight will use its default glow profile");
        } catch (ReflectiveOperationException exception) {
            LOGGER.warn("Could not initialize Swordflight OptiFine shader detection", exception);
        } catch (LinkageError error) {
